...
class SimpleContactsHelper(val context: Context) {
    ...
    fun loadContactImage(
        ...
        val placeholder = if (!isUnknown) placeholderImage ?: BitmapDrawable(context.resources, getContactLetterIcon(placeholderName))
                          else if (placeholderImage != null) BitmapDrawable(context.resources, getCircularBitmapFromDrawable(placeholderImage))
                          else BitmapDrawable(context.resources, getContactLetterIcon(placeholderName))
    ...
    // Vytvoří kruhový bitmapový ořez
